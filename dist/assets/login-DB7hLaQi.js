...
`),confirmButtonText:"ตกลง",confirmButtonColor:"#f59e0b"}),null)}async function w(e,t){window.currentUser=e.user,f("mangpongUser",JSON.stringify(window.currentUser)),await Swal.fire({icon:"success",title:"เข้าสู่ระบบสำเร็จ!",text:`ยินดีต้อนรับ ${window.currentUser.fullName||t}`,confirmButtonText:"ตกลง",confirmButtonColor:"#10b981"}),g("app");const i=document.getElementById("user-display-name");i&&(i.textContent=window.currentUser.fullName||t),window.initializeApp()}async function p(e){await Swal.fire({icon:"error",title:"เข้าสู่ระบบไม่สำเร็จ",text:e&&e.error?e.error:"ข้อมูลการเข้าสู่ระบบไม่ถูกต้อง",confirmButtonText:"ตกลง",confirmButtonColor:"#ef4444"})}async function E(e){console.error("Login error:",e);let t=a.CONNECTION_FAILED,i=r.NETWORK_ERROR;e.message&&e.message.includes("หมดเวลา")?(t=a.TIMEOUT,i="การเชื่อมต่อกับเซิร์ฟเวอร์ใช้เวลานานเกินไป โปรดตรวจสอบการเชื่อมต่ออินเทอร์เน็ตของคุณ"):e.message&&e.message.includes("ไม่สามารถเชื่อมต่อ")?(t=a.CONNECTION_FAILED,i="ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้ โปรดตรวจสอบการเชื่อมต่ออินเทอร์เน็ตของคุณ"):e.message&&e.message.includes("Google Script URL ไม่ได้ถูกกำหนดไว้")?(t="การกำหนดค่าผิดพลาด",i="Google Script URL ไม่ได้ถูกกำหนดไว้ในระบบ โปรดติดต่อผู้ดูแลระบบ"):e.message&&(i=e.message),await Swal.fire({icon:"error",title:t,html:`${i}${e.message&&!i.includes(e.message)?`<br><small class="text-gray-500">(${e.message})</small>`:""}
  ...
  `,confirmButtonText:"ตกลง",confirmButtonColor:"#ef4444"})}async function S(e){const{username:t,password:i}=e;console.log("Attempting login for user:",t),Swal.fire({title:"กำลังเข้าสู่ระบบ...",allowOutsideClick:!1,allowEscapeKey:!1,showConfirmButton:!1,didOpen:()=>{Swal.showLoading()}});try{if(!navigator.onLine){await Swal.fire({icon:"warning",title:"ออฟไลน์",html:`
        ...
        `,confirmButtonText:"ตกลง",confirmButtonColor:"#f59e0b"});return}const n=new Promise((y,c)=>setTimeout(()=>c(new Error(r.TIMEOUT_ERROR)),2e4)),o={action:"login",username:t,password:i};console.log("Sending login request:",o);const l=m(o),s=await Promise.race([l,n]);console.log("Received login response:",s),s&&s.success?await w(s,t):await p(s)}catch(n){await E(n)}}async function x(e){const t=await d(e);t&&await S(t)}export{x as handleLogin};
